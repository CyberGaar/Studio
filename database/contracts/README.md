...
`service-access.json` is the source of truth for group roles and table permissions. `pnpm run generate:roles` creates the reviewed SQL in `database/schema/sql/service-roles.sql`, and `pnpm run check` fails when that SQL or the canonical schema hash drifts.
